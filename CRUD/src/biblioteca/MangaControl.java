package biblioteca;

import java.util.List;

import javafx.beans.property.SimpleStringProperty;
import javafx.beans.property.StringProperty;
import javafx.collections.FXCollections;
import javafx.collections.ObservableList;

public class MangaControl {
	private StringProperty mnome = new SimpleStringProperty("");
	private StringProperty mautor = new SimpleStringProperty("");
	private StringProperty meditora = new SimpleStringProperty("");
	
	private  MangaDAO daoManga = new MangaDAO();
	
	private boolean  editando = false;
	private String nomeAntigo = null;
	private ObservableList<Manga> lista = FXCollections.observableArrayList();
	
	public Manga getEntity() {
		
		Manga m = new Manga ();
		m.setMNome(mnome.get());
		m.setMAutor(mautor.get());
		m.setMEditora(meditora.get());
		return m;
	}
	
	public void setEntity(Manga m) {
		mnome.set(m.getMNome());
		mautor.set(m.getMAutor());
		meditora.set(m.getMEditora());
		
	}
	
	public void limpar() {
		
		mnome.set("");
		mautor.set("");
		meditora.set("");
	}
	
	public void editar () {
		this.editando = true;
		this.nomeAntigo = mnome.get();
	}
	
	public void adicionar () {
		Manga m = getEntity();
		if (this.editando) {
			daoManga.atualizar(nomeAntigo,m);
		} else {
		
		daoManga.create(m);
		}
	}
	
	public void pesquisar() {
		
	List<Manga> tempLista = daoManga.pesquisarPorNome(mnome.get());
	lista.clear();
	lista.addAll(tempLista);
	}
	
	public void delete (Manga m) {
		
		daoManga.delete(m);
		
	}
	
	public StringProperty nomeProperty() {
		
		return mnome;
		
	}
	
	public StringProperty autorProperty() {
		
		return mautor;
		
	}

	public StringProperty editoraProperty() {
	
	return meditora;
	
	}
	public ObservableList<Manga> getLista() {
		return lista;
	}

	public boolean isEditando() {
		return editando;
	}

	public void setEditando(boolean editando) {
		this.editando = editando;
	}

	public String getNomeAntigo() {
		return nomeAntigo;
	}

	public void setNomeAntigo(String nomeAntigo) {
		this.nomeAntigo = nomeAntigo;
	}
	

}
